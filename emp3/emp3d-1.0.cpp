...
#pragma omp parallel for private(f_N2,f_O2,gg,Lelast_N2,Lelast_O2,Lrot_N2,Lrot_O2,Lvib_N2,Lvib_O2,Le)
    ...
    if(t*dt>0.0006) break; //dont do the rest of the time loop if beyond 600us... that data is not representative of our detector. 
    
    int timeWidth = 5;//save every half microseconds time steps. Supposidely integrated over this window.
	...
	if(r[i]>(70000+RE)){//to save the ionospheric data. 
	  for (int j = 1; j < hh; j++) {
	    for (int k = 1; k < pp; k++) { 
	      ...
	      if(n > 1e-9 && n < 1e9 && !TMath::IsNaN(n)) ELVES->Fill(); 
	...
	if(r[i]>(70000+RE)){//to save the ionospheric data. 
	  for (int j = 1; j < hh; j++) {
	    for (int k = 1; k < pp; k++) { 
	      nTOSAVE[i][j][k] += nN22P[i][j][k];
	      //the sum of the the time steps in calculated. 
	      //May need to multiply by dt or divide by overall size of time window. 
	      //May be done in post processing.
	    }
	  }
	}
      }
