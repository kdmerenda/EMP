//1.0  Addition for Auger output of number densities.
//1.1 working version N21P saved
//1.fork N22P saved, Eeff, Er, Et, Ep to diagnose the assymetry in the densities in phi. CREATED nd3tmp, to diagnose the creation of the 3D array
//sampling the grid every 500 time steps for memeory in DEBUGGING
//removed output files from original simulation. 
//corrected some for loop upper bound for Er as recommended by marshall
//added the output of the magnetic field components.  
#include <iostream>
#include <fstream>
#include <complex>
#include <stdio.h>
...
int main()
{
  ...
  DATATYPE nTOSAVE1 [rr][hh][pp];
  DATATYPE nTOSAVE2 [rr][hh][pp];
  DATATYPE nTOSAVE3 [rr][hh][pp];
  DATATYPE nTOSAVE4 [rr][hh][pp];
  DATATYPE nTOSAVE5 [rr][hh][pp];
  DATATYPE nTOSAVE6 [rr][hh][pp];
  DATATYPE nTOSAVE7 [rr][hh][pp];
  DATATYPE nTOSAVE8 [rr][hh][pp];
  double time, n, Eeffsave, Ersave, Epsave, Etsave, Hrsave, Hpsave, Htsave;
  TVector3 vec;
  ELVES->Branch("time",&time,"time/D");
  ELVES->Branch("vec",&vec);
  //  ELVES->Branch("n",&n,"n/D");
  ELVES->Branch("Er",&Ersave,"Er/D");
  ELVES->Branch("Et",&Etsave,"Et/D");
  ELVES->Branch("Ep",&Epsave,"Ep/D");
  ELVES->Branch("Hr",&Hrsave,"Hr/D");
  ELVES->Branch("Ht",&Htsave,"Ht/D");
  ELVES->Branch("Hp",&Hpsave,"Hp/D");
  ELVES->Branch("Eeff",&Eeffsave,"Eeff/D");
...
#pragma omp parallel for private(f_N2,f_O2,gg,Lelast_N2,Lelast_O2,Lrot_N2,Lrot_O2,Lvib_N2,Lvib_O2,Le)
    ...
    //    if(t*dt>0.0006) goto endofbigloop; //dont do the rest of the time loop if beyond 600us... that data is not representative of our detector. 
    
    int timeWidth = 1;//save every half microseconds time steps. Supposidely integrated over this window.
    int dointegration = 0; //doesnt make sense to do integration if the timeWidth is too large!!??
    if (t == 50) {
      logfile.open("log.txt",std::fstream::app);
      logfile << "AUGER OUTPUT INTEGRATION: " << dointegration << endl;
      logfile << "AUGER OUTPUT TIMEWIDTH: " << timeWidth << " time steps." << endl;
      logfile.close();
    }

    if(t%timeWidth==0 && t>0){
      for (int i = 1; i < rr; i++) {
	//	if(r[i]>(50000+RE)){//to save the ionospheric data. 
	  if(r[i]<(10000+RE)){//study the source
	  for (int j = 1; j < hh; j++) {
	    for (int k = 1; k < pp; k++) { 
	      if(dointegration){
		//		n = nTOSAVE1[i][j][k]       + nN22P[i][j][k];   //these arrays need to be added as the calculations of the current time step was not taken without it. 
		Etsave = nTOSAVE2[i][j][k]  + Et[i][j][k];	
		Epsave = nTOSAVE3[i][j][k]  + Ep[i][j][k];	
		Ersave = nTOSAVE4[i][j][k]  + Er[i][j][k];	
		Htsave = nTOSAVE6[i][j][k]  + Ht[i][j][k];	
		Hpsave = nTOSAVE7[i][j][k]  + Hp[i][j][k];	
		Hrsave = nTOSAVE8[i][j][k]  + Hr[i][j][k];	
		Eeffsave = nTOSAVE5[i][j][k]+ Eeff[i][j][k];   
	      }else{
		//		n = nN22P[i][j][k];   //these arrays need to be added as the calculations of the current time step was not taken without it. 
		Etsave =  Et[i][j][k];	
		Epsave =  Ep[i][j][k];	
		Ersave =  Er[i][j][k];	
		Htsave =  Ht[i][j][k];	
		...
		Eeffsave = Eeff[i][j][k];   
	      ...
	      //	      if(n > 1e-14 && n < 1e14 && !TMath::IsNaN(n)) ELVES->Fill(); 
	      ELVES->Fill(); 
	    
	      //reset the array for the next integration
	      nTOSAVE1[i][j][k]=0;
	      nTOSAVE2[i][j][k]=0;
	      nTOSAVE3[i][j][k]=0;
	      nTOSAVE4[i][j][k]=0;
      ...
      for (int i = 1; i < rr; i++) {
	//	if(r[i]>(70000+RE)){//to save the ionospheric data. 
	  for (int j = 1; j < hh; j++) {
	    for (int k = 1; k < pp; k++) { 
	      //		nTOSAVE1[i][j][k] += nN22P[i][j][k];
		nTOSAVE2[i][j][k] += Et[i][j][k];  
		nTOSAVE3[i][j][k] += Ep[i][j][k];
		nTOSAVE4[i][j][k] += Er[i][j][k];
		nTOSAVE6[i][j][k] += Ht[i][j][k];  
		nTOSAVE7[i][j][k] += Hp[i][j][k];
		nTOSAVE8[i][j][k] += Hr[i][j][k];
		nTOSAVE5[i][j][k] += Eeff[i][j][k];
	    }
	  }
      }
      //}
