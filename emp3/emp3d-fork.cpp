//2.0 stripped down version of 1.2, to output solly the number density. 
//2.1 Output other variables of interest in the ELVESTree Parametrer
//2.2 not outputting the vector anymore to check that vector3 is not a problem, it is too much space... now it is jsut radius theta and phi. Also all outputted to float now. 
/// THIS THE WORKING VERSION FOR PRODUCTION. 

//removes any possible integration. 
#include <iostream>
#include <fstream>
#include <complex>
#include <stdio.h>
...
int main()
{
  ...
  fread(&SourceAlt,sizeof(float),1,sourceFile);
  fread(&IPeak,sizeof(float),1,sourceFile);
  fread(&LightType,sizeof(int),1,sourceFile);
  fclose(sourceFile);
  ...
  //------------------------------------------------------------------------//
  //-------OUTPUT TO AUGER DATA FILE----------------------------------------//
  TFile * augerfile = new TFile("augerData.root","RECREATE");  
  TTree *ELVESParameters = new TTree("ELVESParameters","ELVESParameters");

  float nTOSAVE1 [rr][hh][pp];
  float time, n,thetatmp, phitmp,radiustmp;
  TVector3 vec;
  ...
  struct  OutputTreeVariables{
    ...
    // vector<float> EarthMagneticField; //this is a 3D vector ouputing the magnetic field of the earth used for the simulation
    // vector<int> GridSize;             //this is a four dimensional vector giving the number of time and space steps (time, r , th, ph)
    // vector<float> StepSize;           //this is a four dimensional vector giving the size of the steps. (time, r , th, ph) 
  ...
  static OutputTreeVariables myOutput;
  ELVESParameters->Branch("parameters",&myOutput,"PeakCurrent/F:AltSource/F:BRadius/F:BTheta/F:BPhi/F:NStepsTime/I:NStepsRadius/I:NStepsTheta/I:NStepsPhi/I:SizeStepTime/F:SizeStepRadiusLow/F:SizeStepRadiusHigh/F:SizeStepTheta/F:SizeStepPhi/F:TimeIntegratedSteps/I:Orientation/I:LightningType/I:GroundMethod/I:GWave/I");
  myOutput.PeakCurrent        = IPeak ;
  myOutput.AltSource          = SourceAlt;
  myOutput.BRadius            = Br;
  myOutput.BTheta             = Bt;
  myOutput.BPhi               = Bp;
  myOutput.NStepsTime         = tsteps;
  myOutput.NStepsRadius       = rr;
  myOutput.NStepsTheta        = hh;
  myOutput.NStepsPhi          = pp;
  myOutput.SizeStepTime       = dt;
  myOutput.SizeStepRadiusLow  = dr1;
  myOutput.SizeStepRadiusHigh = dr2;
  myOutput.SizeStepTheta      = dth;
  myOutput.SizeStepPhi        = dph;
  myOutput.TimeIntegratedSteps= 4; //hard coded here and below 
  myOutput.Orientation        = sourcedirection;
  myOutput.LightningType      = LightType;
  myOutput.GroundMethod       = groundmethod;
  myOutput.GWave              = dogwave;


  cout << "Lightning: " << LightType << " " << myOutput.LightningType << endl;
  cout << "Ground: " << groundmethod<< " " << myOutput.GroundMethod << endl;
  ELVESParameters->Fill();
  ELVESParameters->Print();
  augerfile->Write();
  delete ELVESParameters;
  
  TTree *ELVES = new TTree("ELVES","ELVES");
  ELVES->Branch("time",&time,"time/F");
  //ELVES->Branch("position",&vec);
  ELVES->Branch("nN22P",&n,"nN22P/F");
  ELVES->Branch("theta",&thetatmp,"theta/F");
  ELVES->Branch("phi",&phitmp,"phi/F");
  ELVES->Branch("radius",&radiustmp,"radius/F");
  ...
  for (int i = 0; i < rr; i++) {
	...
	if (dogwave) {
	  ...
	  ne[i][j][k] = ne[i][j][k] * ( 1 + gwfac );
 	  // cout << gwfac << " " << dogwave << endl;
	  // exit(0);
	} else {
	  nd3[i][j][k] = nd[i];
	  // cout << "actually the conditional is working" << endl;
	  // exit(0);
	}
      }
    }
...
#pragma omp parallel for
	...
	// testfile.open("testfile.txt",std::fstream::app);	      
	for (int i = 0; i < nsalts; i++) {
	  for (int j = st-nextra; j < st+nextra+1; j++) {
	    jfl = (DATATYPE)(j-st);
	    for (int k = sp-nextra; k < sp+nextra+1; k++) {
	      kfl = (DATATYPE)(k-sp);
	      Er[i][j][k] = Er[i][j][k] - c2e[i] * Isource[t][i] * exp(-jfl*jfl/9.0) * exp(-kfl*kfl/9.0) / (PI * 9.0*drange*drange);
	      //  testfile << t*dt << " " << r[i] << " " << th[j] << " " <<  ph[k] << " " << Er[i][j][k] << " " << c2e[i] << " " << Isource[t][i] << " " << jfl << " " << kfl << endl ;
	    }
	  }
	}
	//	testfile.close();
...
#pragma omp parallel for private(f_N2,f_O2,gg,Lelast_N2,Lelast_O2,Lrot_N2,Lrot_O2,Lvib_N2,Lvib_O2,Le)
    ...
    //AUGER
    //Changes to cpp code to save the whole number densities of optical fields
    //Data is appended at everytime steps, and that value needs to be saved.
    //For now, only saving 1 emmission line. 
    //choose steps to narrow the file. 
    //the time steps should be maximized and the data should be integrated over a range that is plausible to save. 
    //    if(t*dt>0.0015) goto endofbigloop; //dont do the rest of the time loop if beyond 600us... that data is not representative of our detector....FALSE!!!! 
    //due to time propaguation one wants to save a lot more than just the first 600us. 

    if (t == 0) {
      logfile.open("log.txt",std::fstream::app);
      logfile << "AUGER OUTPUTS IN PROGRESS (integration)" << endl;
      logfile.close();
    ...
    for (int i = 1; i < rr; i++) {
      for (int j = 1; j < hh; j++) {
	for (int k = 1; k < pp; k++) { 
	  if(t==0)  nTOSAVE1[i][j][k]=0;
	  if(t%4 == 0){
	    n = nTOSAVE1[i][j][k] + nN22P[i][j][k];
	    time = t*dt;
	    // vec.SetXYZ(1.,1.,1.);
	    // vec.SetTheta(th[j]);
	    // vec.SetPhi(ph[k]);
	    ...
	    radiustmp = r[i];
	    phitmp = ph[k]; 
	    if(r[i]>(70000+RE) && n > 1e-14 && n < 1e14 && !TMath::IsNaN(n)){
	      //if(r[i]>(70000+RE)&&r[i]<(95000+RE) && n > 1e-14 && n < 1e14 && !TMath::IsNaN(n)){//to save the ionospheric data
	      ELVES->Fill(); 
	    }
	    nTOSAVE1[i][j][k]=0;  
	  } else {
	    nTOSAVE1[i][j][k] += nN22P[i][j][k];
	  }
	}
      }
    }
