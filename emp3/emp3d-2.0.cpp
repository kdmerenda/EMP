...
//2.1 Output other variables of interest
...
int main()
{
  ...
  ELVESParameters->Branch("parameters",&myOutput,"PeakCurrent/F:AltSource/F:BRadius/F:BTheta/F:BPhi/F:NStepsTime/I:NStepsRadius/I:NStepsTheta/I:NStepsPhi/I:SizeStepTime/F:SizeStepRadiusLow/F:SizeStepRadiusHigh/F:SizeStepTheta/F:SizeStepPhi/F:TimeIntegratedSteps/I:Orientation/I:LightningType/I:GWave/I");
  ...
  myOutput.TimeIntegratedSteps= 1; //hard coded here and below in next 'auger' section. 
...
#pragma omp parallel for private(f_N2,f_O2,gg,Lelast_N2,Lelast_O2,Lrot_N2,Lrot_O2,Lvib_N2,Lvib_O2,Le)
    ...
    if (t == 0) {
      logfile.open("log.txt",std::fstream::app);
      logfile << "AUGER OUTPUTS IN PROGRESS " << endl;
      logfile.close();
    }
	...
	for (int k = 1; k < pp; k++) { 
	  ...
	  if(n > 1e-14 && n < 1e14 && !TMath::IsNaN(n)){
