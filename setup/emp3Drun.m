...
[gsigma,gepsilon] = getGroundParams(in);
in.gsigma = gsigma(1);
...
fwrite(fid,in.sourcealt,'float');
fwrite(fid,in.I0,'float');
fwrite(fid,in.lightningtype,'int');
%in.lightningtype
fclose(fid);
...
in.phmax = in.range / in.Re;
in.dth = in.drange / in.Re;
in.dph = in.drange / in.Re;
...
if in.doIRI,
    ...
    ne = YukiIonosphere((in.r-in.Re)/1000,in.beta,in.hk);
end

% some other possibilities:
%ne = IRIDaytime1((r-in.Re)/1000);
%ne = VictorNeProfile((r-in.Re)/1000,1);
...
if in.dovenus,
    ne = VenusIonosphere1((in.r-in.Re)/1000,2);
...
ds = 1/(sqrt(1/in.dr2^2 + 1/(in.Re*in.dth)^2 + 1/(in.Re*in.dph)^2));
...
rates = getNonlinearRates((in.r-in.Re)/1000,nd,in.nground);
...
ax1 = subplot(221);
plot(ax1,log10(ne),(in.r-in.Re)/1000);
hold(ax1,'on');
...
if (in.submitjob),
    ...
    % submitstr = ['qsub -q ' in.cluster ' -d ' in.rundir ' -l nodes=1:ppn=' in.numnodes ' -l walltime=72:00:00 ' ...   pbsfile];
    
    submitstr = ['./' in.exefile ' &'];
